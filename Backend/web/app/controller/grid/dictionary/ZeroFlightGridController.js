...
app.controller('ZeroFlightGridController', ['$scope',"$rootScope","ZeroFlightRestService","GlobalStorage","$mdDialog",
  function ($scope,$rootScope,ZeroFlightRestService,GlobalStorage,$mdDialog) {
    ...
    $scope.gridOptions.initColumnNameDefs = function () {
      ...
      i++;
      $scope.gridOptions.columnDefs[i].displayName = $scope.lang.words["Parks"];
      i++;
    ...
    $scope.gridOptions.initColumnDefs = function () {
      $scope.gridOptions.columnDefs = [
        ...
        {name: 'parks_id',displayName: '',  enableCellEdit: false, width: 190,cellFilter:"FilterParksNameById",
          sortingAlgorithm:algiritmSortingParksNameById,headerCellClass:"grid-cell-header"
        },
    ...
    if (GlobalStorage.isStorageExist("ZeroFlight"))
    {
      $scope.gridOptions.data =angular.copy(GlobalStorage.getStorage("ZeroFlight"));
    }
    else{
      var resource = ZeroFlightRestService.getResource();
      resource.query({}, function (data, response) {
        GlobalStorage.setStorage("ZeroFlight",data);
    ...
    function algiritmSortingParksNameById (a, b, rowA, rowB, direction) {
      var a_name = getParksNameById(a).toLowerCase();
      var b_name = getParksNameById(b).toLowerCase();
      if ( a_name < b_name )
        return -1;
      if ( a_name > b_name )
        return 1;
