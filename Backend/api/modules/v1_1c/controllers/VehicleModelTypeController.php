...
use app\api\modules\v1_1c\models\VehicleModelType;

class VehicleModelTypeController extends ActiveController
{
    public $modelClass = 'app\api\modules\v1_1c\models\VehicleModelType';
                        ...
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            try {
                                if (isset($item->sync_1c_id)) {
                                    $vehicleModelType = VehicleModelType::getRecordBy1CId($item->sync_1c_id);
                                }
                                else{
                            ...
                            if (empty($vehicleModelType)) {
                                $resultAdd = VehicleModelType::addRecord($item);
                                $result[$k]['result'] = $resultAdd;
                                $result[$k]['requestType'] = 'create';

                            } else {
                                $resultUpdate = VehicleModelType::updateRecord($item, $vehicleModelType);
                    ...
                    case 'delete': {
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            if (empty($vehicleModelType)) {
                                ...
                                $resultDelete = VehicleModelType::deleteRecord($vehicleModelType->getPrimaryKey());
