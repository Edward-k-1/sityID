...
use app\api\modules\v1_1c\models\Worker;

class WorkerController extends ActiveController
{
    public $modelClass = 'app\api\modules\v1_1c\models\Worker';
                        ...
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            try {
                                if (isset($item->sync_1c_id)) {
                                    $worker = Worker::getRecordBy1CId($item->sync_1c_id);
                                }
                                else{
                            ...
                            if (empty($worker)) {
                                $resultAdd = Worker::addRecord($item);
                                $result[$k]['result'] = $resultAdd;
                                $result[$k]['requestType'] = 'create';

                            } else {
                                $resultUpdate = Worker::updateRecord($item, $worker);
                    ...
                    case 'delete': {
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            if (empty($worker)) {
                                ...
                                $resultDelete = Worker::deleteRecord($worker->getPrimaryKey());
