...
use app\api\modules\v1_1c\models\Park;

class ParkController extends ActiveController
{
    public $modelClass = 'app\api\modules\v1_1c\models\Park';
                        ...
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            try {
                                if (isset($item->sync_1c_id)) {
                                    $park = Park::getRecordBy1CId($item->sync_1c_id);
                                }
                                else{
                            ...
                            if (empty($park)) {
                                $resultAdd = Park::addRecord($item);
                                $result[$k]['result'] = $resultAdd;
                                $result[$k]['requestType'] = 'create';

                            } else {
                                $resultUpdate = Park::updateRecord($item, $park);
                    ...
                    case 'delete': {
                        foreach ($dataFrom1C->data as $item) {
                            ...
                            if (empty($park)) {
                                ...
                                $resultDelete = Park::deleteRecord($park->getPrimaryKey());
